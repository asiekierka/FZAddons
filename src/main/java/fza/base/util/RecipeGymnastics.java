...
public class RecipeGymnastics {
	...
	public static void initBase() {
		...
		HashSet<Item> forbiddenItems = new HashSet<Item>();
		forbiddenItems.add(Items.diamond);
		forbiddenItems.add(Items.redstone);
		forbiddenItems.add(Items.coal);
		forbiddenItems.add(Items.emerald);
		...
		while(it.hasNext()) {
			...
			if(forbiddenItems.contains(r.output.getItem()) || (outputName != null && (outputName.startsWith("oreNether") || (outputName.startsWith("ore") && !OreDictionaryUtil.forbiddens.contains(outputName.substring(3)))))) {
				it.remove();
			}
		}
		...
		GameRegistry.addRecipe(new ShapelessOreRecipe(new ItemStack(Items.paper, 3), new Object[]{"dustWood", "dustWood", "dustWood"}));
		GameRegistry.addRecipe(new ShapelessOreRecipe(new ItemStack(Items.paper, 3), new Object[]{"pulpWood", "pulpWood", "pulpWood"}));
		...
		if(!OreDictionary.getOres("oreNetherLapis").isEmpty()) {
			TileEntityGrinder.addRecipe("oreNetherLapis", new ItemStack(Items.dye, 1, 4), 25.5F);
		}
		
		if(OreDictionary.getOres("oreDiamond").isEmpty()) {
			TileEntityGrinder.addRecipe(Blocks.diamond_ore, new ItemStack(Items.diamond), 2.5F);
		}else {
			TileEntityGrinder.addRecipe("oreDiamond", new ItemStack(Items.diamond), 2.5F);
		}
		if(!OreDictionary.getOres("oreNetherDiamond").isEmpty()) {
			TileEntityGrinder.addRecipe("oreNetherDiamond", new ItemStack(Items.diamond), 5F);
		}
		
		if(OreDictionary.getOres("oreEmerald").isEmpty()) {
			TileEntityGrinder.addRecipe(Blocks.emerald_ore, new ItemStack(Items.emerald), 2.5F);
		}else {
			TileEntityGrinder.addRecipe("oreEmerald", new ItemStack(Items.emerald), 2.5F);
		}
		if(!OreDictionary.getOres("oreNetherEmerald").isEmpty()) {
			TileEntityGrinder.addRecipe("oreNetherEmerald", new ItemStack(Items.emerald), 5F);
		}
		
		if(OreDictionary.getOres("oreCoal").isEmpty()) {
			TileEntityGrinder.addRecipe(Blocks.coal_ore, new ItemStack(Items.coal), 2.5F);
		}else {
			TileEntityGrinder.addRecipe("oreCoal", new ItemStack(Items.coal), 2.5F);
		}
		if(!OreDictionary.getOres("oreNetherCoal").isEmpty()) {
			TileEntityGrinder.addRecipe("oreNetherCoal", new ItemStack(Items.coal), 5F);
		}
		
		if(OreDictionary.getOres("oreRedstone").isEmpty()) {
			TileEntityGrinder.addRecipe(Blocks.redstone_ore, new ItemStack(Items.redstone), 9.5F);
		}else {
			TileEntityGrinder.addRecipe("oreRedstone", new ItemStack(Items.redstone), 9.5F);
		}
		if(!OreDictionary.getOres("oreNetherRedstone").isEmpty()) {
			TileEntityGrinder.addRecipe("oreNetherRedstone", new ItemStack(Items.redstone), 25.5F);
		}
		
		if(OreDictionary.getOres("cloth").isEmpty()) {
			for(int i = 0; i < 16; i++) {
				OreDictionary.registerOre("cloth", new ItemStack(Blocks.wool, 1, i));
			}
		}
		TileEntityGrinder.addRecipe("cloth", new ItemStack(Items.string), 3.5F);
	...
	private static void doSlagPrep() {
		...
		if(!OreDictionary.getOres("oreSphalerite").isEmpty() && !OreDictionary.getOres("ingotZinc").isEmpty()) {
			for(ItemStack sphal : OreDictionary.getOres("oreSphalerite")) {
				TileEntitySlagFurnace.SlagRecipes.register(sphal.copy(), 1.2F, OreDictionary.getOres("ingotZinc").get(0).copy(), .4F, Blocks.stone);
			}
		}
		...
		if(!OreDictionary.getOres("orePyrite").isEmpty() && !OreDictionary.getOres("ingotIron").isEmpty()) {
			for(ItemStack pyr : OreDictionary.getOres("orePyrite")) {
				TileEntitySlagFurnace.SlagRecipes.register(pyr.copy(), 1.2F, OreDictionary.getOres("ingotIron").get(0).copy(), .4F, Blocks.netherrack);
			}
		}
			...
			if(!ores.isEmpty() && !ConfigurationHandler.smeltingBlackList.contains("ore"+s)) {
				...
				if(!ingots.isEmpty()) {
					for(ItemStack stack : ores) {
						...
						TileEntitySlagFurnace.SlagRecipes.register(stack.copy(), 1.2F*mult, ingots.get(0).copy(), .4F, Blocks.stone);
					}
					ArrayList<ItemStack> nethers = OreDictionary.getOres("oreNether"+s);
					if(!nethers.isEmpty()) {
						for(ItemStack stack : nethers) {
							TileEntitySlagFurnace.SlagRecipes.register(stack.copy(), 2.4F, ingots.get(0).copy(), .4F, Blocks.netherrack);
						}
					}
				}
			}
			...
			if(s.startsWith("dirtyGravel") && !OreDictionary.getOres(s).isEmpty()) {
				...
				if(!ConfigurationHandler.smeltingBlackList.contains(s)) {
					...
					if(!ingots.isEmpty()) {
						...
						TileEntitySlagFurnace.SlagRecipes.register(dirty, 1.1F, ingot, .2F, new ItemStack(Blocks.dirt));
						FurnaceRecipes.smelting().func_151394_a(dirty, ingot, 0.1F);
					}
				}
				...
				if(!OreDictionary.getOres("ingot"+postfix).isEmpty()) {
					...
					ItemStack ingot = OreDictionary.getOres("ingot"+postfix).get(0).copy();
					FurnaceRecipes.smelting().func_151394_a(clean, ingot, 0.1F);
				}
			}
				...
				if(!ConfigurationHandler.smeltingBlackList.contains(s) && !OreDictionary.getOres("ingot"+postfix).isEmpty()) {
					ItemStack ingot = OreDictionary.getOres("ingot"+postfix).get(0).copy();
					FurnaceRecipes.smelting().func_151394_a(reduced, ingot, 0.1F);
				}
			}
				...
				if(!OreDictionary.getOres("ingot"+postfix).isEmpty()) {
					...
					ItemStack ingot = OreDictionary.getOres("ingot"+postfix).get(0).copy();
					FurnaceRecipes.smelting().func_151394_a(crystal, ingot, 0.1F);
				}
			}
